...
WORKDIR /templates/service/host/Base.Host
RUN dotnet restore
RUN dotnet ef migrations script -i -o migrations-Base.sql
...
COPY --from=build /templates/service/host/Base.Host/migrations-Base.sql migrations-Base.sql
